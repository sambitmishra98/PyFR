...
def build_graph(con, elewts_fn, exwts=None):
    if exwts is None:
        exwts = {}
...
def graph_edge_sources(vtab, idtype=None):
    nv = len(vtab) - 1
    if idtype is None:
        idtype = graph_idtype(nv)

    return np.repeat(np.arange(nv, dtype=idtype), np.diff(vtab))


def build_csr(nv, src, dst, ewts=None, symmetrise=False, dedup=False):
    idt = graph_idtype(nv)
    src = np.asarray(src, dtype=idt)
    dst = np.asarray(dst, dtype=idt)

    if ewts is None:
        ewts = np.ones(len(src), dtype=np.int32)
    else:
        ewts = np.asarray(ewts, dtype=np.int32)

    if symmetrise:
        osrc, odst, oewts = src, dst, ewts
        src = np.concatenate([osrc, odst])
        dst = np.concatenate([odst, osrc])
        ewts = np.concatenate([oewts, oewts])

    if dedup and len(src):
        src, dst, ewts = dedup_weighted_edges(src, dst, ewts, nv)
    ...
    if len(src):
        ...
        vtab = np.zeros(nv + 1, dtype=int)
        vtab[1:] = np.bincount(src, minlength=nv).cumsum()
    else:
        vtab = np.zeros(nv + 1, dtype=int)
        dst = np.empty(0, dtype=idt)
        ewts = np.empty(0, dtype=np.int32)

    return vtab, dst, ewts


class GraphPartitioner:
    def __init__(self, *, seed=2079, nrefine=10, ufactor=0.01, niter=1,
                 nvcycles=4, coarsen_min_mult=20, coarsen_min_abs=500,
                 coarsen_stop=0.8, rb_limit=64, init_nparts_small=16,
                 greedy_restarts_sc=4, greedy_restarts_mc=8):
        self.nrefine = nrefine
        self.ufactor = ufactor
        self.niter = niter
        self.nvcycles = nvcycles
        self.coarsen_min_mult = coarsen_min_mult
        self.coarsen_min_abs = coarsen_min_abs
        self.coarsen_stop = coarsen_stop
        self.rb_limit = rb_limit
        self.init_nparts_small = init_nparts_small
        self.greedy_restarts_sc = greedy_restarts_sc
        self.greedy_restarts_mc = greedy_restarts_mc
    ...
    def _bounds(self, partwts, vwts):
        ...
        upper = np.maximum(targets*(1 + self.ufactor), np.ceil(targets))
        lower = max(1 - self.ufactor, 0)*targets
    ...
    def _boundary_gains(rows, parts, boundary, nv=0):
        ...
        b_gains[b_idx, b_src] = -(nv if nv else b_own.max() + 1)
    ...
    def partition(self, graph, partwts):
        nv, ne = len(graph.vtab) - 1, len(graph.etab)
        idt = graph_idtype(max(nv, ne))

        vtab = graph.vtab.astype(idt)
        etab = graph.etab.astype(idt)
        ewts = graph.ewts.astype(np.int32, copy=False)
        vwts = graph.vwts.squeeze().astype(np.int32, copy=False)
        ...
        edge_src = graph_edge_sources(vtab, idt)

        return self._multilevel_kway(vtab, etab, ewts, vwts, partwts,
                                     edge_src, idt)

    def coarsen(self, vtab, etab, ewts, vwts):
        idt = graph_idtype(len(vtab) - 1)
        edge_src = graph_edge_sources(vtab, idt)
        return self._coarsen(vtab, etab, ewts, vwts, edge_src, idt)

    def refine(self, parts, vtab, etab, vwts, partwts, nrefine=None):
        if nrefine is None:
            nrefine = self.nrefine

        edge_src = graph_edge_sources(vtab)
        ewts = np.ones(len(etab), dtype=np.int32)
        partwts = np.asarray(partwts, dtype=float)
        self._refine_kway(parts, vtab, etab, ewts, vwts, partwts, edge_src,
                          nrefine=nrefine)

    def _multilevel_kway(self, vtab, etab, ewts, vwts, partwts, edge_src, idt):
        ...
        min_size = max(self.coarsen_min_mult*nparts, self.coarsen_min_abs)
        cstop = self.coarsen_stop
        ...
        while True:
            ...
            result = self._coarsen(c_vtab, c_etab, c_ewts, c_vwts, c_esrc, idt)
        ...
        def score(p):
            cut = c_ewts[p[c_esrc] != p[c_etab]].sum()
        ...
        parts = self._greedy_kway(c_vtab, c_etab, c_ewts, c_vwts, partwts,
                                  c_esrc, idt)
        best_score = score(parts)

        if nparts <= self.rb_limit:
            rb_parts = self._rb_kway(c_vtab, c_etab, c_ewts, c_vwts, partwts,
                                     idt)
        ...
        for i in range(nlevels - 1, -1, -1):
            f_vtab, f_etab, f_ewts, f_vwts, _, f_esrc = hierarchy[i]
            _, _, _, _, match, _ = hierarchy[i + 1]

            parts = parts[match]

            self._greedy_refine(parts, f_vtab, f_etab, f_ewts, f_vwts,
                                partwts, f_esrc)

            if self.nrefine > 0:
                self._refine_kway(parts, f_vtab, f_etab, f_ewts, f_vwts,
                                  partwts, f_esrc)

        # Apply multigrid
        if len(hierarchy) > 2:
            vc_nrefine = min(self.nrefine, 5)
            nvcycles = self.nvcycles

            f_vtab, f_etab, f_ewts, f_vwts, _, f_esrc = hierarchy[0]
            _, f_upper, _ = self._bounds(partwts, f_vwts)

            def vc_score(p):
                cut = f_ewts[p[f_esrc] != p[f_etab]].sum()
                if f_vwts.ndim > 1:
            ...
            for _ in range(nvcycles):
                ...
                for i in range(len(hierarchy) - 2, -1, -1):
                    lv = hierarchy[i]
                    lv_vtab, lv_etab, lv_ewts, lv_vwts, _, lv_esrc = lv
                    _, _, _, _, match, _ = hierarchy[i + 1]

                    parts = parts[match]

                    self._greedy_refine(parts, lv_vtab, lv_etab, lv_ewts,
                                        lv_vwts, partwts, lv_esrc)

                    if vc_nrefine > 0:
                        self._refine_kway(parts, lv_vtab, lv_etab, lv_ewts,
                                          lv_vwts, partwts, lv_esrc,
                                          nrefine=vc_nrefine)
    ...
    def _greedy_kway(self, vtab, etab, ewts, vwts, partwts, edge_src, idt):
        nv, nparts = len(vtab) - 1, len(partwts)
        ...
        # Multiple restarts
        nrestarts = self.greedy_restarts_mc if mc else self.greedy_restarts_sc
        if nparts > self.init_nparts_small:
            nrestarts = max(1, nrestarts // 2)

        best = (np.inf, np.inf, None)
        for attempt in range(nrestarts):
            seeds = self._pick_seeds(vtab, etab, nparts, idt, attempt)
            parts = self._grow_kway(vtab, etab, ewts, vwts, targets, seeds)

            cut = ewts[parts[edge_src] != parts[etab]].sum()
    ...
    def _rb_kway(self, vtab, etab, ewts, vwts, partwts, idt):
        nv, nparts = len(vtab) - 1, len(partwts)
        ...
        half = pw.sum() / 2
        cumwt = np.cumsum(pw)
        split = np.searchsorted(cumwt, half, side='right')
        split = max(1, min(split, nparts - 1))
        left_wt = pw[:split].sum()
        ratio = left_wt / pw.sum()

        # Bisect the graph
        bparts = self._bisect(vtab, etab, ewts, vwts, ratio, idt)
        ...
        for side, prange in [(0, range(split)), (1, range(split, nparts))]:
            ...
            if npk == 1:
                ...
                sv, se, sew = self._extract_subgraph(vidx, vtab, etab, ewts,
                                                     idt)
                sub_vwts = vwts[vidx]
                sub_pw = pw[prange.start:prange.stop]
                sub_parts = self._rb_kway(sv, se, sew, sub_vwts, sub_pw, idt)
    ...
    def _pick_seeds(self, vtab, etab, k, idt, attempt=0):
        nv = len(vtab) - 1
        if k >= nv:
            return np.arange(k, dtype=int) % nv

        if attempt == 0:
            start = int(np.diff(vtab).argmin())
        else:
            start = int(self._rng.integers(nv))

        # BFS from start to find a peripheral vertex
        dist = self._bfs(start, vtab, etab, idt)
        dist[dist < 0] = 0
        seeds = [dist.argmax()]

        # Greedily pick farthest vertices from existing seeds
        min_dist = self._bfs(seeds[0], vtab, etab, idt)
        min_dist[min_dist < 0] = nv

        for _ in range(k - 1):
            next_seed = min_dist.argmax()
            seeds.append(next_seed)
            d = self._bfs(next_seed, vtab, etab, idt)
            d[d < 0] = nv
            np.minimum(min_dist, d, out=min_dist)

        return np.array(seeds)

    def _grow_kway(self, vtab, etab, ewts, vwts, targets, seeds):
        ...
        for p, s in enumerate(seeds.tolist()):
            parts[s] = p
            avail[s] = 0
            part_wts[p] = vwts[s]
    ...
    def _extract_subgraph(self, vidx, vtab, etab, ewts, idt):
        nv_sub = len(vidx)
        if not nv_sub:
        ...
        if total == 0:
            return (np.zeros(nv_sub + 1, dtype=int),
                    np.array([], dtype=idt), np.array([], dtype=np.int32))
        ...
        all_ew = ewts[starts[v_idx] + np.arange(total) - base]
        ...
        sub_ewts = all_ew[valid]

        return sub_vtab, sub_etab, sub_ewts

    def _coarsen(self, vtab, etab, ewts, vwts, edge_src, idt):
        nv = len(vtab) - 1
        if nv <= 1:
            return None
        ...
        if len(etab) == 0:
            return (vtab.copy(), etab.copy(), ewts.copy(), vwts.copy(),
                    np.arange(nv, dtype=idt), edge_src.copy())

        # Heavy edge matching
        match = self._heavy_edge_match(vtab, etab, edge_src, ewts, idt)
        nv_c = match.max() + 1
    ...
    def _bisect(self, vtab, etab, ewts, vwts, target_ratio, idt):
        nv = len(vtab) - 1
        if nv <= 1:
            return np.zeros(nv, dtype=np.int32)
        ...
        for i in range(min(nstarts, max(1, nv // 10))):
            if i == 0:
                seed = int(np.diff(vtab).argmin())
                start = int(self._bfs(seed, vtab, etab, idt).argmax())
            else:
                start = int(self._rng.integers(nv))

            parts = self._bisect_ggp(start, vtab, etab, ewts, svwts, target_wt)
    ...
    def _bisect_ggp(self, start, vtab, etab, ewts, vwts, target_wt):
        nv = len(vtab) - 1
        ...
        while cur_wt < target_wt:
            ...
            if not candidates.any():
                ...
                if len(remaining) == 0:
                    break
                v = int(remaining[self._rng.integers(len(remaining))])
            else:
                cidx = np.flatnonzero(candidates)
                cgains = gain[cidx]
                max_g = cgains.max()
                top = cidx[cgains == max_g]
                v = int(top[self._rng.integers(len(top))])
    ...
    def _bfs(self, start, vtab, etab, idt):
        nv = len(vtab) - 1
        levels = np.full(nv, -1, dtype=np.int32)
        levels[start] = 0

        frontier = np.array([start], dtype=idt)
        level = 1
    ...
    def _greedy_refine(self, parts, vtab, etab, ewts, vwts, partwts, edge_src):
        nv, nparts = len(vtab) - 1, len(partwts)
    ...
    def _mc_greedy_pass(self, parts, vtab, etab, ewts, vwts, nbr_wgt, nbr_cnt,
                        bm, ubvec, minpwgts, maxpwgts, degrees, part_wts,
                        omode):
        ...
        b_src, b_dst, b_gain, b_own = self._boundary_gains(nbr_cnt[boundary],
                                                             parts, boundary)
    ...
    def _refine_kway(self, parts, vtab, etab, ewts, vwts, partwts, edge_src,
                     nrefine=None):
        nv, nparts = len(vtab) - 1, len(partwts)
        if nv <= nparts or len(etab) == 0:
            return

        if nrefine is None: nrefine = self.nrefine
        ...
        nbr_wgt = None
        nbr_cnt = np.zeros((nv, nparts), dtype=np.int32)
        ...
        if vwts.ndim > 1:
            ...
            for _ in range(nrefine):
                ...
                if imb_diff > 0.02:
                    ...
                    self._mc_greedy_pass(parts, vtab, etab, ewts, vwts,
                                         nbr_wgt, nbr_cnt, bm, ubvec,
                                         minpwgts, maxpwgts_b, degrees,
                                         part_wts, 'balance')
                ...
                moved[:] = False
                improved = self._fm_pass_edge_cut(
                    parts, vtab, etab, ewts, vwts, nbr_wgt, nbr_cnt,
                    mc_upper, lower, nparts, degrees, moved
                )
            ...
            for _ in range(nrefine):
                ...
                improved = self._fm_pass_edge_cut(
                    parts, vtab, etab, ewts, vwts, nbr_wgt, nbr_cnt,
                    upper, lower, nparts, degrees, moved
                )
                if not improved:
                    break

    def _fm_pass_edge_cut(self, parts, vtab, etab, ewts, vwts,
                          nbr_wgt, nbr_cnt, upper, lower, nparts,
                          degrees, moved):
        nv = len(vtab) - 1
        part_wts = self._wbincount(parts, vwts, nparts)
        ...
        _, _, b_best_gain, _ = self._boundary_gains(nbr_cnt[boundary], parts,
                                                    boundary, nv)
...
def dedup_weighted_edges(src, dst, wts, nv=None):
    nv = nv or max(src.max(), dst.max()) + 1
    eid = src.astype(np.int64)*nv + dst
    order = np.argsort(eid)
    eid = eid[order]
    diff = np.empty(len(eid), dtype=bool)
    diff[0] = True
    np.not_equal(eid[1:], eid[:-1], out=diff[1:])
    labels = np.cumsum(diff) - 1
    d_wts = np.maximum(np.bincount(labels, wts[order]), 1).astype(np.int32)
    return src[order][diff], dst[order][diff], d_wts


def con_to_flat(con, etype_off):
    cidx_off = np.empty(max(con.cidxmap) + 1, dtype=int)
    for cidx, (etype, _) in con.cidxmap.items():
        if etype in etype_off:
            cidx_off[cidx] = etype_off[etype]

    return cidx_off[con.cidxs] + con.eidxs
