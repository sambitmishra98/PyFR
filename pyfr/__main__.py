...
from pyfr.partitioners import (BasePartitioner, get_partitioner,
                               reconstruct_by_diffusion, reconstruct_by_relocation,
...
def main():
    ...
    # Reconstruct partitioning by relocation from existing partitioning
    ap_partition_relocate = ap_partition.add_parser(
        'relocate', help='partition relocate --help'
    )

    ap_partition_relocate.add_argument('mesh', help='input mesh file')
    ap_partition_relocate.add_argument('np', 
                                      help='number of partitions or a colon '
                                           'delimited list of weights')
    ap_partition_relocate.add_argument('name', help='Existing partitioning name')
    ap_partition_relocate.add_argument('dpname', help='Relocated partitioning name')
    ap_partition_relocate.add_argument(
        '-f', '--force', action='count', help='overwrite existing partitioning'
    )

    ap_partition_relocate.set_defaults(
        process=process_partition_relocate
    )
...
def process_partition_diffuse(args):
    ...
    # Reconstruct the partitioning used in the solution
    vparts = reconstruct_by_diffusion(read_only_mesh, args.name, pwts, args.progress)
...
def process_partition_relocate(args):
    # Validate partitioning name
    if not re.match(r'\w+$', args.name):
        raise ValueError('Invalid partitioning name')
