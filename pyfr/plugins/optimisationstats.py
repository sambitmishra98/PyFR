...
class OptimisationStatsPlugin(BaseSolnPlugin):
    ...
    def collect_stats(self, intg):
        ...
        for plugin in intg.plugins:
            if plugin.name == 'dtaustats':
                dtaustats = plugin
                break
        else:
            raise ValueError("dtaustats plugin not found")

        # Here, if intg data from plugin exists, we take data from there 
        if self.Δτ_controller == 'local-pi' and dtaustats.dtau_stats:
            t1['max-Δτ'] = dtaustats.dtau_stats['max']['all']
            t1['min-Δτ'] = dtaustats.dtau_stats['min']['all']
            t1[     'n'] = dtaustats.dtau_stats[ 'n' ]['all']
